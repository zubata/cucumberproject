...
public class BasePage {
    ...
    public void fillfield(String xpath, String name) {
        WebElement field = waiting.until(ExpectedConditions.elementToBeClickable(By.xpath(xpath)));
    ...
    public void multiplechoice(String xpath,String choice) {
        WebElement order = waiting.until(ExpectedConditions.elementToBeClickable(By.xpath(xpath)));
        Select select = new Select(order);
        select.selectByVisibleText(choice);
    }

    @Step("Нажатие на радиокнопку {xpath}")
    ...
    public void pageWaitingLoad() {
        WebElement temp = driver.findElement(By.xpath("//body"));
        waitelement(temp);
    }
